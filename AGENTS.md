...
client) that exposes `codebuddy` as 4 MCP tools. mcode loads
`mcp.json` on session start; the wrapper keeps one
`codebuddy --acp` subprocess alive for the session.

Version: 0.3.0. Status: stable, in production use.
...
| `bin/codebuddy-mcp-server.py` | The stdio MCP wrapper (~430 lines, one long-lived subprocess) | Adding tools, changing ACP protocol handling, changing log format |
...
| `tests/mcp-features-test.py` | 4-tool end-to-end (status, list_tasks, prompt, continue, model, append respawn) | After adding/changing tools |
| `tests/test_mcp_wrapper_unit.py` | 16 unit tests (no subprocess) | After changing wrapper internals |
...
2. **Run the test suite** (all three; all should pass before commit):
   ```bash
   python3 -m unittest tests.test_mcp_wrapper_unit   # 16 tests, ~4ms
...
- Don't reintroduce `bin/install.sh`, `bin/invoke-codebuddy*`, or
  `tests/smoke.sh` — they were removed in 0.3.0 because the MCP
  path makes them dead code. See CHANGELOG 0.3.0 "Removed" section.
...
- Per-tool usage breakdown (prompt vs continue vs status vs
  list_tasks) in `state/mcp-YYYY-MM-DD.log`.
- Consider supporting `streamable-http` transport in `mcp.json` for
  remote codebuddy deployments (currently stdio-only).
