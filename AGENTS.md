...
client) that exposes `codebuddy` as 9 MCP tools. mcode loads
...
2. **Run the test suite** (all four; all should pass or skip cleanly before commit):
   ```bash
   python3 -m unittest tests.test_mcp_wrapper_unit   # 55 tests, ~220ms
...
- **Wrapper code**: Python 3.10+ compatible, no type-annotation
  ...
  background reader thread, and the async submit/poll API uses a
  `_task_lock` for task state).
...
- **Streamable-HTTP transport in `mcp.json`** for remote codebuddy
  deployments (currently stdio-only). One worker per server = real
  concurrent codebuddy calls. See memory note about why this is
  the path to real concurrency.
- **Add a `health` tool** that reports `codebuddy --acp` reachability
  separately from process liveness.
- **Per-tool usage breakdown** in `state/mcp-YYYY-MM-DD.log`
  (already partially captured in `list_tasks`; just needs an
  aggregator).
