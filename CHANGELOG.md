...
- `skills/codebuddy-integration/SKILL.md`: frontmatter `description`
  ...
  the existing 0.2.1 mode / `--keep` / `--status` / `--metrics` content
  is preserved verbatim under "Quick reference (legacy CLI)" as a
  fallback for clients that drive text reasoning through the bash
  tool. `metadata.version` is bumped to 0.3.0.
- `README.md`: top-of-file summary now leads with the 0.3.0 spec
  changes; legacy CLI usage is moved to a clearly-labeled "0.2.1 (legacy
  CLI)" section.

### Kept (legacy fallback, removed in 0.4.0)

- `bin/invoke-codebuddy` (CLI, sync), `bin/invoke-codebuddy-acp-worker.py`
  (CLI worker), `bin/invoke-codebuddy-bridge.sh` (sync bridge for
  `task(run_in_background=true)`), `bin/install.sh` (no mcp registration
  step needed under spec 1.0.0), and `tests/smoke.sh` /
  `tests/full-smoke.sh` (legacy CLI smoke tests) are **all retained
  unchanged** in 0.3.0. They will be removed in 0.4.0 once the MCP
  path is the only supported entry point.

### Not changed (out of scope)

- `assets/mcode-base-system-prompt.md` — content unchanged, only its
  path is referenced by `mcp.json`.
- `docs/architecture.md` — describes 0.2.1's invoke-codebuddy flow; a
  0.3.0 architecture update is left for a follow-up.
- `.minimax-plugin/plugin.json` — the mcode client-extension file is
  regenerated by `mcode install`, not stored in this repo.
